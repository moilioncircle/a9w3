...
function parseCommand($argv){
    ...
    switch($argv[1]){
        ...
        case 'ftpxfp':
            ...
            break;
        case 'httpfp':
            command_httpfp($cmndarg,$options);
            break;
        case 'help':
        default:
            command_help();
...
function command_httpfp($cmndarg,$options){
    if(count($cmndarg)<2){
        echo 'bad parameter: difffp';
        exit;
    }
    ...
    foreach($get_arr as $v){
        ...
        if(!is_dir(dirname($lclfile))){
            $tkpt = '/';
            $crpt = PATH_ROOT;
            foreach(explode($tkpt,dirname($v)) as $dn){
                if(empty($dn)) continue;
                $crpt.= $dn.$tkpt;
                if(!is_dir($crpt)){
                    mkdir($crpt);
        ...
        $rmthand = fopen($server.'/'.$v, "rb");
        $bstring = stream_get_contents($rmthand);
        fclose($rmthand);
        if($bstring){
            $lclhand = fopen($lclfile, "wb");
            if(fwrite($lclhand, $bstring) !== FALSE){
                echo $v."  OK\n";
            }
            fclose($lclhand);
            continue;
    ...
    command:
        ...
        * ftpxfp [option] difffp user[:passwd]@server[:port] a9w3
            ...
            -rmt=op     op=get|del|nop ,get by default ('<<' operation).
                           ...
                           nop: skip all.
        * httpfp difffp server
            useing http to get file remote by difffp.
            difffp      made by 'difffp'.
            server      the a9w3 root url of the http server.
        * help
