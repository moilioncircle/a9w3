...
function drawArticleListLabelView(cur,obj,lst){
    ...
    for(var i=0;i<lst.length;i++){
        if(cur != lst[i]){
            buff.push("<a href='javascript:{showPage(\"label\",\""+lst[i]+"\")}' class='a9w3_link_infos'>"+parent.W3GUI.ARTICLE_LABEL.getValue(lst[i])+"</a>");
        }else{
            buff.push(parent.W3GUI.ARTICLE_LABEL.getValue(lst[i]));
        }
        buff.push(" ");
    }
    buff.push("</td>");
