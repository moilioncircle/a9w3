...
function drawGalleryListMonthView(cur,obj,lst){
    if(obj != null) listCache['obj']=obj;
    else obj = listCache['obj'];
...
function drawGalleryLinkView(page,obj,lst){
    ...
    for(var j=0;j<show.length;j++){
        ...
        if(show[j]!=page){
            buff.push("<a href='javascript:drawGalleryLinkView("+show[j]+")' class='a9w3_link_infos'>"+show[j]+"</a>");
        }else{
    ...
    //
    for(var i=0;i<size;i++){
        parent.W3GUI.getGalleryItem(lst[poff+i],function(ai){
            var buff = [];
